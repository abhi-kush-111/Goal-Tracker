  ...
  return (
    <div
      ...
      className={cn(
        ...
        isDragging && "opacity-50 border-orange-500/50 bg-orange-500/5 z-50",
      )}
    >
...
const DraggableCalendarMilestone = ({
  milestone,
  goalTitle,
}: {
  milestone: Milestone;
  ...
  return (
    <div
      ...
      {...attributes}
      className={cn(
        "px-1 md:px-2 py-0.5 md:py-1 rounded bg-orange-500/10 border border-orange-500/20 cursor-grab active:cursor-grabbing touch-none",
        isDragging && "opacity-50 border-orange-500/50 bg-orange-500/5 z-50",
      )}
    >
      <p className="text-[8px] md:text-[9px] font-bold text-orange-500 drop-shadow-[0_0_8px_rgba(255,87,34,0.6)] truncate leading-tight">
        {milestone.title}
      </p>
    </div>
  );
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-1 md:mb-2">
        <span
          className={cn(
            "text-[10px] md:text-[10px] font-semibold tracking-widest uppercase",
            isToday(day)
        ...
        {milestones.length > 0 && (
          <span className="text-[8px] md:text-[9px] font-semibold tracking-widest uppercase dark:text-stone-500 text-stone-600 px-1 md:px-1.5 py-0.5 dark:bg-white/5 bg-stone-100 rounded">
            {milestones.length}
          </span>
        )}
      </div>
...
const AssignTasksView = ({
  ...
  const { isOver: isUnassignedOver, setNodeRef: setUnassignedNodeRef } =
    useDroppable({
      id: "unassigned",
    });
    ...
    >
      ...
      >
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-5 gap-4 md:gap-6 min-h-0">
          ...
          <Card
            className={cn(
              "lg:col-span-1 flex flex-col h-[200px] lg:h-auto min-h-0 transition-colors",
              isUnassignedOver && "bg-white/[0.05] border-white/20",
            )}
          >
            <div className="p-3 md:p-4 border-bottom dark:border-white/5 border-stone-200 dark:bg-white/[0.02] bg-stone-50">
              <h3 className="text-[10px] md:text-[10px] font-semibold tracking-widest uppercase uppercase tracking-widest dark:text-stone-500 text-stone-600">
                Unassigned Milestones
              </h3>
            </div>
            <div
              ref={setUnassignedNodeRef}
              className="flex-1 overflow-y-auto p-3 md:p-4 space-y-2 md:space-y-3 custom-scrollbar"
            >
          ...
          <Card className="lg:col-span-4 p-2 md:p-6 flex flex-col min-h-0 overflow-hidden">
            <div className="grid grid-cols-7 mb-2 md:mb-4">
              {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
                <div
                  key={day}
                  className="text-center text-[8px] md:text-[9px] font-semibold tracking-widest uppercase uppercase tracking-widest dark:text-stone-500 text-stone-600 py-1 md:py-2"
                >
                  {day}
                </div>
              ))}
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-1 md:pr-2">
              <div className="grid grid-cols-7 gap-1 md:gap-3">
                {calendarDays.map((day) => {
                  ...
                  const dayMilestones = allAssignedMilestones.filter(
                    (m) => m.due_date === dayStr,
                  );
                  return (
                    <DroppableDay
...
export default function App() {
  ...
  const [activeCalendarDragId, setActiveCalendarDragId] = useState<
    string | null
  >(null);
  ...
  const handleAddMilestone = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const updatedGoal = await storage.addMilestone({
        ...newMilestone,
  ...
  const stats = useMemo(() => {
    ...
    const completedMilestones = goals.reduce(
      (acc, g) =>
        acc +
        (g.milestones || []).filter((m) => m.done === 1 || m.done === true)
          .length,
      0,
    );
      ...
      >
        <svg width={size} height={size} className="transform -rotate-90">
          <circle
            ...
            fill="transparent"
            className="text-stone-200 dark:text-white/5"
          />
          <motion.circle
            cx={size / 2}
            cy={size / 2}
        ...
        <div className="absolute flex flex-col items-center justify-center">
          <span
            className={cn(
              "font-bold dark:text-white text-stone-900",
  ...
  return (
    <div className="flex min-h-screen dark:bg-[#242422] bg-white font-sans selection:bg-orange-500/30">
      ...
      <aside
        className={cn(
          "fixed lg:static inset-y-0 left-0 w-64 border-r dark:border-white/5 border-stone-200 dark:bg-[#0a0e1a] bg-stone-50 flex flex-col h-screen z-50 transition-transform duration-300",
          isMobileMenuOpen
            ? "translate-x-0"
            : "-translate-x-full lg:translate-x-0",
        )}
      >
        <div className="p-6 border-bottom dark:border-white/5 border-stone-200 flex justify-between items-center">
          <LogoFull />
          <button
            className="lg:hidden dark:text-stone-500 text-stone-600 hover:dark:text-white text-stone-900"
            onClick={() => setIsMobileMenuOpen(false)}
          >
            <X className="w-5 h-5" />
          </button>
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto custom-scrollbar">
          {isCustomizingNav ? (
            ...
            >
              {navOrder.map((item) => {
                const Icon =
                  { Sun, LayoutDashboard, Target, Filter, Calendar }[
                    item.icon
                  ] || LayoutDashboard;
                return (
                  <Reorder.Item
                    key={item.id}
                    value={item}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-xl dark:bg-white/5 bg-stone-100 border dark:border-white/10 border-stone-300 cursor-grab active:cursor-grabbing touch-none"
                  >
                    <GripVertical className="w-4 h-4 dark:text-stone-500 text-stone-600" />
                    <Icon className="w-4 h-4 dark:text-stone-400 dark:text-stone-500 text-stone-600" />
                    <span className="text-sm font-semibold dark:text-stone-200 text-stone-800">
                      {item.label}
                    </span>
                  </Reorder.Item>
                );
            ...
            navOrder.map((item) => {
              const Icon =
                { Sun, LayoutDashboard, Target, Filter, Calendar }[item.icon] ||
                LayoutDashboard;
              return (
                <button
                  key={item.id}
                  ...
                  className={cn(
                    ...
                    view === item.id
                      ? "bg-orange-500/10 text-orange-500 drop-shadow-[0_0_8px_rgba(255,87,34,0.6)]"
                      : "dark:text-stone-500 text-stone-600 hover:dark:bg-white/5 hover:bg-stone-100 dark:hover:text-stone-200 hover:text-stone-900",
                  )}
                >
          ...
          <div className="pt-6 pb-2 px-3 flex items-center justify-between">
            <p className="text-[9px] font-semibold tracking-widest uppercase uppercase tracking-widest dark:text-stone-500 text-stone-600">
              My Goals
            </p>
            <button
              onClick={() => setIsCustomizingNav(!isCustomizingNav)}
              className="dark:text-stone-500 text-stone-600 hover:dark:text-stone-300 text-stone-700"
            >
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? "dark:bg-white/5 bg-stone-100 dark:text-white text-stone-900"
                    : "dark:text-stone-500 text-stone-600 hover:dark:bg-white/5 hover:bg-stone-100 dark:hover:text-stone-200 hover:text-stone-900",
                )}
              >
                ...
                {isActive && (
                  <motion.div
                    ...
                    className="absolute left-0 w-1 h-4 bg-orange-500 rounded-full"
                  />
                )}
              </button>
            );
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl dark:text-stone-500 text-stone-600 hover:bg-orange-500/5 hover:text-orange-500 drop-shadow-[0_0_8px_rgba(255,87,34,0.6)] transition-all duration-200 mt-4 border border-dashed dark:border-white/5 border-stone-200 hover:border-orange-500/20"
          >
        ...
        <div className="p-6 border-t dark:border-white/5 border-stone-200 space-y-4">
          <div className="flex items-center justify-between px-2">
            <span className="text-xs font-semibold dark:text-stone-500 text-stone-600">
              Theme
            </span>
            <button
              onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
              className="p-2 rounded-lg dark:bg-white/5 bg-stone-100 hover:dark:bg-white/10 bg-stone-200 dark:text-stone-300 text-stone-700 transition-colors"
            >
              {theme === "dark" ? (
          ...
          {supabase && session && (
            <div className="flex items-center justify-between px-2">
              ...
              <button
                ...
                className="text-[10px] font-semibold tracking-widest uppercase text-rose-500 hover:text-rose-400 transition-colors px-2 py-1"
              >
                Sign Out
              </button>
            </div>
          ...
          <div className="dark:bg-orange-500/5 bg-orange-50/50 border dark:border-orange-500/10 border-orange-500/10 rounded-2xl p-4">
            <div className="flex justify-between items-end mb-2">
              <span className="text-[9px] font-semibold tracking-widest uppercase uppercase tracking-wider text-orange-500 drop-shadow-[0_0_8px_rgba(255,87,34,0.6)]/60">
                Overall
              </span>
              <span className="text-sm font-bold text-orange-500 drop-shadow-[0_0_8px_rgba(255,87,34,0.6)]">
                {stats.avgProgress}%
              </span>
            </div>
            <div className="h-1.5 dark:bg-white/5 bg-stone-100 rounded-full overflow-hidden">
              <motion.div
                initial={{ width: 0 }}
                animate={{ width: `${stats.avgProgress}%` }}
                className="h-full bg-orange-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]"
              />
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 overflow-y-auto h-screen custom-scrollbar flex flex-col relative w-full">
        {/* Mobile Header */}
        <div className="lg:hidden flex items-center justify-between p-4 border-b dark:border-white/5 border-stone-200 dark:bg-[#0a0e1a] bg-stone-50 sticky top-0 z-30">
          <LogoFull />
          <button
            onClick={() => setIsMobileMenuOpen(true)}
            className="p-2 dark:text-stone-400 dark:text-stone-500 text-stone-600 hover:dark:text-white text-stone-900 dark:bg-white/5 bg-stone-100 rounded-lg"
          >
          ...
          {view === "today" ? (
            ...
            >
              ...
              {todayMilestones.length === 0 ? (
                <div className="space-y-6 md:space-y-8">
                    ...
                    <div className="flex justify-between items-start mb-8">
                      <div>
                        ...
                        <p className="text-white/60 text-sm">
                          Your schedule is clear — assign tasks from the
                          Calendar
                        </p>
                      </div>
                      <div className="text-right">
                        ...
                        {todayProgress !== yesterdayProgress && (
                          ...
                          >
                            {todayProgress > yesterdayProgress ? "↑" : "↓"} vs
                            yesterday
                          </div>
                        )}
                      </div>
                  ...
                  <div className="md:hidden flex items-center justify-between bg-white/40 dark:bg-[#1a1a1a]/40 backdrop-blur-md border border-stone-200/50 dark:border-white/5 rounded-2xl p-3 mb-4">
                    ...
                    {todayProgress !== yesterdayProgress && (
                      ...
                      >
                        {todayProgress > yesterdayProgress ? "↑" : "↓"} vs yday
                      </div>
                    )}
                  </div>
                ...
                <div className="flex flex-col items-center justify-center min-h-[60vh] rounded-[2rem] bg-[var(--sidebar)] relative overflow-hidden border border-orange-500/20">
                  ...
                  <div className="text-center mb-12">
                    <div className="text-7xl font-black text-[var(--accent)] mb-2">
                      {todayProgress}%
                    </div>
                  ...
                  <div className="flex flex-wrap justify-center gap-4 mb-12">
                    <div className="px-6 py-3 rounded-full bg-white/5 border border-white/10 text-white font-medium">
                      {todayMilestones.length} tasks done
                    </div>
                    <div className="px-6 py-3 rounded-full bg-white/5 border border-white/10 text-white font-medium">
                      {Math.max(0, ...goals.map((g) => g.streak))} day streak
                    </div>
                    <div className="px-6 py-3 rounded-full bg-white/5 border border-white/10 text-white font-medium">
                      100% progress
                    </div>
                    ...
                    <button
                      onClick={async () => {
                        navigator.clipboard.writeText(
                          `✅ GoalForge Daily Summary — ${format(new Date(), "MMM d")}\n${todayProgress}% completed\n${todayMilestones.length} tasks completed\nStreak: ${Math.max(0, ...goals.map((g) => g.streak))} days`,
                        );
                      }}
                      className="px-8 py-4 bg-white/10 text-white rounded-2xl font-bold hover:bg-white/20 transition-colors"
                    >
                      Copy Summary
                    </button>
                    <button
                      onClick={() => setView("calendar")}
                      className="px-8 py-4 bg-[var(--accent)] text-black rounded-2xl font-bold hover:opacity-90 transition-opacity"
                    >
                      Plan Tomorrow →
                    </button>
                  </div>
                  ...
                  <div className="space-y-6 md:space-y-8">
                    ...
                    >
                      ...
                      >
                        <div
                          className={cn(
                            "absolute top-0 left-0 h-full rounded-full transition-all duration-800 ease-[cubic-bezier(0.16,1,0.3,1)] flex items-center justify-end pr-2",
                            todayProgress === 100 && "animate-pulse",
                          )}
                          style={{
                            width: `${todayProgress}%`,
                            backgroundColor: getBarColor(todayProgress),
                          }}
                        >
                          {todayProgress > 5 && (
                        ...
                        if (highestStreak > 0) {
                          ...
                          return (
                            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-500 text-sm font-medium">
                              {isUrgent ? (
                                <>
                                  ⚡ {incompleteCount} tasks left — {hoursLeft}{" "}
                                  hours to protect your {highestStreak}-day
                                  streak
                                </>
                              ) : (
                                <>
                                  🔥 {highestStreak} day streak active —
                                  complete today's tasks to keep it alive
                                </>
                              )}
                            </div>
                          );
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      <div className="lg:col-span-2 space-y-8">
                        {(() => {
                          const incompleteTasks = todayMilestones.filter(
                            (m) => !m.done,
                          );
                          if (incompleteTasks.length === 0 && !showBreather)
                            return null;
                          ...
                          const currentTask =
                            sortedTasks[carouselIndex % sortedTasks.length];
                          const catColor =
                            currentTask?.categoryColor || "#10b981";

                          return (
                            <div className="flex flex-col items-center">
                              <div className="relative w-full max-w-md aspect-square rounded-[2rem] overflow-hidden bg-white dark:bg-[#1a1a1a] shadow-2xl shadow-black/10 dark:shadow-black/40 border border-stone-100 dark:border-white/5">
                                <AnimatePresence mode="wait">
                                  {showBreather ? (
                                    <motion.div
                                      key="breather"
                                      ...
                                      transition={{ duration: 0.3 }}
                                      className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center bg-orange-500 text-white"
                                    >
                                      <motion.div
                                        initial={{ scale: 0 }}
                                        animate={{ scale: 1 }}
                                    ...
                                    >
                                      <div className="flex justify-between items-start mb-auto">
                                        <div
                                          className="px-3 py-1.5 rounded-full text-[9px] font-semibold tracking-widest uppercase uppercase tracking-widest"
                                          style={{
                                        ...
                                        {currentTask?.priority === "High" && (
                                          <div className="flex items-center gap-1 text-rose-500 text-[9px] font-semibold tracking-widest uppercase uppercase tracking-widest">
                                      ...
                                      <div className="flex-1 flex flex-col justify-center items-center text-center">
                                        <h2 className="text-3xl md:text-4xl font-black text-stone-900 dark:text-white leading-tight tracking-tight mb-4">
                                          {currentTask?.title}
                                        </h2>
                                      ...
                                      <div className="mt-auto pt-8">
                                        <button
                                          onClick={(e) =>
                                            handleArenaComplete(currentTask, e)
                                          }
                                          ...
                                          style={{
                                            backgroundColor: catColor,
                                            color: "#fff",
                                            boxShadow: `0 10px 25px -5px ${catColor}40`,
                                          }}
                                        >
                              ...
                              {!showBreather && sortedTasks.length > 1 && (
                                <div className="flex items-center gap-4 mt-8">
                                  <button
                                    ...
                                    className="hidden md:flex w-10 h-10 rounded-full bg-white dark:bg-white/5 border border-stone-200 dark:border-white/10 items-center justify-center text-stone-400 hover:text-stone-900 dark:hover:text-white transition-colors"
                                  >
                                    ...
                                    {sortedTasks.map((_, idx) => (
                                      <div
                                        ...
                                        className={cn(
                                          ...
                                          idx ===
                                            carouselIndex % sortedTasks.length
                                            ? "bg-stone-800 dark:bg-white w-4"
                        ...
                        {todayMilestones.filter((m) => m.done).length > 0 && (
                          <div className="border border-stone-200 dark:border-white/10 rounded-2xl overflow-hidden">
                            <button
                              onClick={() =>
                      ...
                      <div className="lg:col-span-1">
                        <div className="lg:hidden mb-4">
                          <button
                            ...
                            className="w-full py-3 rounded-xl border border-stone-200 dark:border-white/10 text-sm font-bold text-stone-600 dark:text-stone-300 flex items-center justify-center gap-2"
                          >
                            {showMomentumMobile
                              ? "Hide Momentum"
                              : "Show Momentum"}
                        ...
                        <div
                          className={cn(
                            "space-y-6",
                            !showMomentumMobile && "hidden lg:block",
                          )}
                        >
                          <div className="p-6 rounded-[1.5rem] bg-white dark:bg-white/[0.02] border border-stone-100 dark:border-white/5">
                            <div className="flex items-center gap-2 mb-2">
                              <Flame className="w-5 h-5 text-orange-500 drop-shadow-[0_0_8px_rgba(255,87,34,0.6)]" />
                              <span className="text-[9px] font-semibold tracking-widest uppercase uppercase tracking-widest text-stone-400">
                                Daily Streak
                              </span>
                            </div>
                            {Math.max(0, ...goals.map((g) => g.streak)) > 0 ? (
                              <>
                                <div className="text-5xl font-black text-stone-900 dark:text-white mb-1">
                                  {Math.max(0, ...goals.map((g) => g.streak))}
                                </div>
                                <p className="text-xs text-stone-500">
                                  {
                                    goals.reduce((prev, current) =>
                                      prev.streak > current.streak
                                        ? prev
                                        : current,
                                    ).title
                                  }{" "}
                          ...
                          <div className="p-6 rounded-[1.5rem] bg-white dark:bg-white/[0.02] border border-stone-100 dark:border-white/5">
                            <div className="flex items-center justify-between mb-4">
                              ...
                              <button
                                ...
                                className="text-[9px] font-semibold tracking-widest uppercase text-orange-500 drop-shadow-[0_0_8px_rgba(255,87,34,0.6)] hover:text-orange-600"
                              >
                                see all →
                              </button>
                            </div>
                          ...
                          <div className="p-6 rounded-[1.5rem] bg-white dark:bg-white/[0.02] border border-stone-100 dark:border-white/5">
                            <span className="text-[9px] font-semibold tracking-widest uppercase uppercase tracking-widest text-stone-400 mb-4 block">
                              Progress Breakdown
                            </span>
                            ...
                            <div className="pt-4 border-t border-stone-100 dark:border-white/5 flex justify-between items-end">
              ...
              <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 md:mb-10 gap-4">
                <div>
                  ...
                  <p className="dark:text-stone-400 dark:text-stone-500 text-stone-600 text-sm">
                    ...
                    <span className="text-orange-500 drop-shadow-[0_0_8px_rgba(255,87,34,0.6)] font-bold">
                    ...
                    switch (widget.id) {
                      case "stats":
                        return (
                          <div
                            key="stats"
                            className="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8"
                          >
                            {[
                              {
                                label: "Total Goals",
                      ...
                      case "progress":
                        return (
                          <Card
                            key="progress"
                            className="lg:col-span-2 p-8 mb-8"
                            delay={0.2}
                          >
                      ...
                      case "repeatability":
                        return (
                          <Card
                            key="repeatability"
                            className="lg:col-span-3 p-8 mb-8"
                            delay={0.25}
                          >
                      ...
                      case "categories":
                        return (
                          <Card
                            key="categories"
                            className="lg:col-span-1 p-8 mb-8"
                            delay={0.25}
                          >
                                ...
                                todayMilestones.map((ms) => (
                                  ...
                                  >
                                    <button
                                      onClick={async () => {
                                        if (ms.isHabit) {
                                          const updatedHabit = await storage.toggleHabit(
                                            ms.id,
                                            new Date(),
                                          );
                                          if (updatedHabit) setHabits(prev => prev.map(h => h.id === updatedHabit.id ? updatedHabit : h));
                                        } else if (ms.isGoalAsMilestone) {
                                          const updatedGoal = await storage.toggleGoalCompletion(
                                            ms.id,
                                            new Date(),
                                          );
                                          if (updatedGoal) setGoals(prev => prev.map(g => g.id === updatedGoal.id ? updatedGoal : g));
            ...
            >
              <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 md:mb-10 gap-4">
                ...
                <button
                  ...
                  className="w-full md:w-auto flex items-center justify-center gap-2 px-6 py-3 bg-orange-500 text-[#431407] btn-extruded rounded-2xl font-bold text-sm hover:bg-orange-400 transition-all shadow-lg shadow-orange-500/20"
                >
                  <Plus className="w-4 h-4" />
                  Create New Goal
                </button>
              ...
              {goals.length === 0 ? (
                <div className="py-32 text-center border border-dashed dark:border-white/5 border-stone-200 rounded-[40px] dark:bg-white/[0.01] bg-stone-50">
                  ...
                  <button
                    onClick={() => setIsAddingGoal(true)}
                    className="px-8 py-3 bg-orange-500 text-[#431407] btn-extruded rounded-2xl font-bold text-sm hover:bg-orange-400 transition-colors"
                  >
                    Create First Goal
                  </button>
                </div>
                  ...
                  {goals.map((goal, i) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="group dark:bg-[#2A2A28] bg-white border dark:border-white/5 border-stone-200 rounded-[32px] p-6 flex flex-col hover:dark:bg-white/[0.03] hover:bg-stone-100 hover:dark:border-white/10 hover:border-stone-300 transition-all duration-300 relative overflow-hidden"
                      >
                        <div className="flex justify-between items-start mb-6">
                          <div className="flex items-center gap-4">
                            <div className="w-14 h-14 rounded-2xl dark:bg-white/5 bg-stone-100 flex items-center justify-center text-3xl group-hover:scale-110 transition-transform duration-500">
                              {cat.icon}
                            </div>
                            <div>
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-[9px] font-semibold tracking-widest uppercase dark:text-stone-500 text-stone-600 uppercase tracking-widest">
                                  {goal.category}
                                </span>
                              ...
                              <h4 className="text-lg font-bold dark:text-white text-stone-900 truncate max-w-[200px]">
                                {goal.title}
                              </h4>
                            </div>
                          </div>
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              className="p-2 dark:text-stone-500 text-stone-600 hover:text-orange-500 drop-shadow-[0_0_8px_rgba(255,87,34,0.6)] hover:bg-orange-500/10 rounded-xl transition-all"
                              title="Edit Goal"
                            >
                              <Settings className="w-4 h-4" />
                            </button>
                            <button
                              ...
                              className="p-2 dark:text-stone-500 text-stone-600 hover:dark:text-white text-stone-900 hover:dark:bg-white/5 bg-stone-100 rounded-xl transition-all"
                              title="View Details"
                            >
                            ...
                            <button
                              ...
                              className="p-2 dark:text-stone-500 text-stone-600 hover:text-rose-500 hover:bg-rose-500/10 rounded-xl transition-all"
                              title="Delete Goal"
                            >
                        ...
                        <div className="mt-auto">
                          <div className="flex justify-between items-end mb-3">
                            <div>
                              <div className="text-2xl font-black dark:text-white text-stone-900 font-mono leading-none">
                                {goal.progress}%
                              </div>
                              <div className="text-[9px] font-semibold tracking-widest uppercase dark:text-stone-500 text-stone-600 uppercase tracking-widest mt-1">
                                Progress
                              </div>
                            </div>
                            <div className="text-right">
                              {goal.repeat && goal.repeat !== "None" ? (
                                <>
                                  <div className="text-[10px] font-semibold tracking-widest uppercase dark:text-stone-300 text-stone-700">
                                    {goal.completed_dates?.length || 0}
                                  </div>
                                  <div className="text-[9px] font-semibold tracking-widest uppercase dark:text-stone-500 text-stone-600 uppercase tracking-widest">
                                    Completions
                                  </div>
                                </>
                              ) : (
                          ...
                          <div className="h-2 dark:bg-white/5 bg-stone-100 rounded-full overflow-hidden">
                            <motion.div
                              initial={{ width: 0 }}
                        ...
                        <div className="mt-6 flex items-center justify-between pt-6 border-t dark:border-white/5 border-stone-200">
                          <div className="flex items-center gap-2 text-[9px] font-semibold tracking-widest uppercase dark:text-stone-500 text-stone-600 uppercase">
                          ...
                          {(!goal.repeat || goal.repeat === "None") && (
                            <button
                              ...
                              className="text-[9px] font-semibold tracking-widest uppercase text-orange-500 drop-shadow-[0_0_8px_rgba(255,87,34,0.6)] uppercase tracking-widest hover:underline"
                            >
                              Manage Milestones
                            </button>
                          )}
                  ...
                  <div className="flex flex-col lg:flex-row gap-10">
                    <div className="flex-1">
                      <header className="mb-10">
                        ...
                        <h2 className="text-4xl font-extrabold dark:text-white text-stone-900 tracking-tight mb-4">
                          {activeGoal.title}
                        </h2>
                        {activeGoal.note && (
            ...
            >
              <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 md:mb-10 gap-4">
                ...
                <button
                  ...
                  className="w-full md:w-auto px-6 py-3 bg-orange-500 text-[#431407] btn-extruded rounded-2xl font-bold text-sm hover:bg-orange-400 transition-all shadow-lg shadow-orange-500/20 flex items-center justify-center gap-2"
                >
                ...
                {habits.length === 0 ? (
                  <div className="col-span-full py-20 text-center">
                    <div className="w-20 h-20 bg-orange-500/10 rounded-full flex items-center justify-center mx-auto mb-6">
                      <Activity className="w-10 h-10 text-orange-500 drop-shadow-[0_0_8px_rgba(255,87,34,0.6)]" />
                    </div>
                    <h3 className="text-xl font-bold dark:text-white text-stone-900 mb-2">
                      No habits yet
                    </h3>
                    <p className="dark:text-stone-500 text-stone-600 mb-8">
                      Start small. Consistency is the key to mastery.
                    </p>
                    <button
                      onClick={() => setIsAddingHabit(true)}
                      className="px-6 py-3 bg-orange-500/10 text-orange-500 drop-shadow-[0_0_8px_rgba(255,87,34,0.6)] rounded-xl font-bold text-sm hover:bg-orange-500/20 transition-colors"
                    >
                  ...
                  habits.map((habit) => {
                    ...
                    return (
                      <Card
                        key={habit.id}
                        className="p-6 group hover:dark:border-white/10 hover:border-stone-300 transition-all duration-300"
                      >
                        <div className="flex justify-between items-start mb-6">
                          <div
                          ...
                          <div className="flex gap-1">
                            <button
                              ...
                              className="p-2 dark:text-stone-500 text-stone-400 hover:dark:text-white hover:text-stone-900 transition-colors"
                            >
                              <Settings className="w-4 h-4" />
                            </button>
                            <button
                              onClick={() => handleDeleteHabit(habit.id)}
                              className="p-2 dark:text-stone-500 text-stone-400 hover:text-rose-500 transition-colors"
                            >
                              <Trash2 className="w-4 h-4" />
                            </button>
                          </div>
                        ...
                        <h4 className="text-lg font-bold dark:text-white text-stone-900 mb-1 group-hover:text-orange-500 drop-shadow-[0_0_8px_rgba(255,87,34,0.6)] transition-colors">
                          {habit.title}
                        </h4>
                        <p className="text-xs dark:text-stone-500 text-stone-600 mb-6 uppercase tracking-widest font-bold">
                          {habit.repeat} • {habit.category}
                        </p>

                        <div className="flex items-center justify-between mb-6">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 bg-orange-500/10 rounded-xl flex items-center justify-center text-orange-500 drop-shadow-[0_0_8px_rgba(255,87,34,0.6)]">
                              <Flame className="w-5 h-5" />
                            </div>
                            <div>
                              <p className="text-[10px] font-semibold tracking-widest uppercase dark:text-stone-500 text-stone-600 uppercase tracking-tighter">
                                Streak
                              </p>
                              <p className="text-xl font-black dark:text-white text-stone-900">
                                {habit.streak} Days
                              </p>
                            </div>
                          </div>
                          <button
                            ...
                            className={cn(
                              ...
                              isDoneToday
                                ? "bg-orange-500 text-[#431407] btn-extruded shadow-orange-500/20"
                                : "dark:bg-white/5 bg-stone-100 dark:text-stone-500 text-stone-400 hover:dark:bg-white/10 hover:bg-stone-200",
                            )}
                          >
                            {isDoneToday ? (
                        ...
                        {habit.due_date && (
                          <div className="pt-4 border-t dark:border-white/5 border-stone-200">
                            <div className="flex justify-between items-center text-[9px] font-semibold tracking-widest uppercase uppercase tracking-widest dark:text-stone-500 text-stone-600">
                              <span>Target Date</span>
                              <span>
                                {format(
                                  parseISO(habit.due_date),
                                  "MMM dd, yyyy",
                                )}
                              </span>
                            </div>
                                  ...
                                  <button
                                    onClick={async () => {
                                      if (ms.isHabit) {
                                        ...
                                        const updatedGoal = await storage.toggleMilestone(
                                          ms.id,
                                          selectedDate,
                                        );
                                        if (updatedGoal) setGoals(prev => prev.map(g => g.id === updatedGoal.id ? updatedGoal : g));
            ...
            >
              <div className="p-6 md:p-8">
                ...
                <form onSubmit={handleAddGoal} className="space-y-6">
                  <div>
                    <label className="block text-[9px] font-semibold tracking-widest uppercase uppercase tracking-widest dark:text-stone-500 text-stone-600 mb-2">
                      Goal Title
                    </label>
                    <input
                      autoFocus
                        ...
                        >
                          {categories.map((cat) => (
                            <option
                              key={cat.id}
                              value={cat.name}
                              className="dark:bg-[#2A2A28] bg-white"
                            >
                              {cat.name}
                            </option>
                          ))}
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      value={newGoal.note}
                      onChange={(e) =>
                        setNewGoal({ ...newGoal, note: e.target.value })
                      }
                    />
                  </div>
                ...
                <form onSubmit={handleAddHabit} className="space-y-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewHabit({ ...newHabit, due_date: e.target.value })
                      }
                    />
                  </div>
                  <div className="flex gap-4 pt-4">
